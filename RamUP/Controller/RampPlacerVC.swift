        ...
        if let ramp = selectedRamp {
            if gesture.state == .ended {
                ...
                if gesture.view === rotateBtn {
                    let rotate = SCNAction.repeatForever(SCNAction.rotateTo(x: 0, y: CGFloat(0.08 * Double.pi), z: 0, duration: 0.1))
                    ramp.runAction(rotate)
                } else if gesture.view === upBtn {
                    let up = SCNAction.repeatForever(SCNAction.moveBy(x: 0, y: 0.88, z: 0, duration: 0.1))
                    ramp.runAction(up)
                } else if gesture.view === downBtn {
                    let down = SCNAction.repeatForever(SCNAction.moveBy(x: 0, y: -0.88, z: 0, duration: 0.1))
                    ramp.runAction(down)
                }
            }
        }
